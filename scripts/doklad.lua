...
function sendReport()
    ...
    if not areFieldsFilled() then
        sampAddChatMessage("Заполните все поля в меню докладов!", 0xFF0000)
        isActive = false
    ...
    sampSendChat("/r Докладывает: "..u8:decode(nameStr)..". Пост: "..u8:decode(postStr).." | Состояние: "..u8:decode(conditionStr))
...
imgui.OnFrame(function() return WinState[0] end,
    function()
        ...
        if imgui.InputText(u8"Ваше имя", name, 256) then autoSave() end
        if imgui.InputText(u8"Название поста", post, 256) then autoSave() end
        if imgui.InputText(u8"Состояние поста", condition, 256) then autoSave() end
        ...
        if imgui.Checkbox(u8"Авто /time после доклада", checkboxAutoTime) then autoSave() end
        if imgui.Checkbox(u8"Авто F8 после доклада", checkboxAutoF8) then autoSave() end
        
        imgui.Spacing()
        imgui.Separator() 
        imgui.Spacing()
        
        imgui.Text(u8"Интервал между докладами (секунды):")
        ...
        if isActive then
            ...
            imgui.Text(u8"Следующий доклад через: "..string.format("%d мин. %d сек.", minutes, seconds))
        end
        ...
        imgui.Text(u8"/doklad - активация скрипта")
        imgui.Text(u8"/dokladoff - деактивация скрипта")
...
function main()
    ...
    sampRegisterChatCommand("doklad", function()
        ...
        if isActive then
            ...
            local minutes = math.floor(timeRemaining / 60)
            local seconds = timeRemaining % 60
            sampAddChatMessage(string.format("Следующий доклад через: %d мин. %d сек.", minutes, seconds), 0x00AAFF)
        else
            isActive = true
            sendReport()
        end
    ...
    sampRegisterChatCommand("dokladoff", function()
        isActive = false
        sampAddChatMessage("Автоматические доклады отключены.", 0x00AAFF)
    end)
